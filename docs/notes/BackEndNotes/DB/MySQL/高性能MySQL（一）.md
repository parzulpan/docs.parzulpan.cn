...
* `READ UNCOMMITTED`(**未提交读**)：事务中的修改，即使没有提交，对其他事务也都是可见的。事务可以读取未提交的数据，这也被成为**脏读**(D`irty Read`)。这种级别会导致很多问题，从性能上讲，它不会比其他级别好很多，在实际应用一般很少使用它。
...
可以认为MVCC是行级锁的一个变种，但是它在很多情况下都避免了加锁操作，因此开销更低。虽然实现机制有所不同哦in，但大都实现了非阻塞的读操作，写操作也只锁定必要的行。
